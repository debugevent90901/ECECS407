...
def compute_inverse(context, x):
    ...
    rx = beaver_multiply(context, r, x)
    RX = context.open_share(rx)
    R = context.open_share(r)
...
def get_random_bit(context):
    ...
    # TODO: Your code goes here
    r_square = beaver_multiply(context, r, r)
    R_square = context.open_share(r_square)

    R_alternative = prime_mod_sqrt(R_square.n, Fp.p)
    # convert 1 or -1 to 1 or 0
    b = (1 + r/Fp(R_alternative))/Fp(2)
...
def compute_powers(context, x, k):
    ...
    # TODO: your code goes here
    inv_rs = [compute_inverse(context, r) for r in rs]
    accum = Fp(1)
    for i in range(1, k+1):
        _temp = beaver_multiply(context, rs[i-1], x)
        d = beaver_multiply(context, _temp, inv_rs[i])
        D = context.open_share(d)
        accum *= D
        xi = accum * beaver_multiply(context, inv_rs[0], rs[i])
        xis.append(xi)
...
def random_extract(context, avals):
    ...
    # TODO: your code goes here
    pts = [(Fp(i), avals[i-1]) for i in range(1, len(avals)+1)]
    phi = Poly.interpolate(pts)
    bs = [phi(Fp(i+1)) for i in range(context.n, 2*context.n-context.f)]
    return bs
